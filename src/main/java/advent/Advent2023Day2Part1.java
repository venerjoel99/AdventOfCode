...
public class Advent2023Day2Part1 implements AdventSolver {
    ...
    public void solve() {
        if (exceptionHit) {
            ...
            CubeSet inventory = new CubeSet();
            inventory.insertCubes("red", 12);
            inventory.insertCubes("green", 13);
            inventory.insertCubes("blue", 14);
            int answer = lines.stream()
                    .map(GameFactory::createGame)
                    .filter(Objects::nonNull)
                    .filter(game -> game.isGamePossible(inventory))
                    .map(Game::getGameId)
